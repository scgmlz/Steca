...
void MainWin::initMenus() {
  ...
  addActions(menuFile_,
             {
                 acts_.addFiles, acts_.remFile, separator(), acts_.enableCorr,
                 acts_.remCorr, separator(), acts_.loadSession,
  ...
  addActions(menuImage_,
             {
                 acts_.rotateImage, acts_.mirrorImage, acts_.fixedIntenImage,
                 acts_.linkCuts, acts_.showOverlay, acts_.stepScale,
                 acts_.showBins,
             });

  addActions(menuDgram_,
             {
                 acts_.selRegions, acts_.showBackground, acts_.clearBackground,
                 acts_.clearReflections, separator(), acts_.addReflection,
                 acts_.remReflection, separator(), acts_.combinedDgram,
                 acts_.fixedIntenDgram,
             });

  addActions(menuOutput_,
             {
                 acts_.outputPolefigures, acts_.outputDiagrams,
                 acts_.outputDiffractograms,
             });
...
void MainWin::addActions(QMenu* menu, QList<QAction*> actions) {
  ...
  for (auto action : actions) action->setToolTip(prefix + action->toolTip());
}
...
void MainWin::checkUpdate(bool completeReport) {
  ...
  connect(reply, &QNetworkReply::finished, [this, completeReport, reply]() {
    if (QNetworkReply::NoError != reply->error()) {
      ...
      if (ver != lastVer)
        messageDialog(str("%1 update").arg(name),
...
void MainWin::onShow() {
  ...
  auto safeLoad = [this](rcstr fileName) {
    ...
    if (info.exists()) hub_.loadSession(info);
  };
  ...
  if (s.readBool(config_key::STARTUP_CHECK_UPDATE, true)) checkUpdate(false);
  if (s.readBool(config_key::STARTUP_ABOUT, true)) about();
}
