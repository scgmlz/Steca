...
QRgb intenImage(inten_t inten, inten_t maxInten, bool curved) {
  ...
  if (curved && inten > 0) inten = qPow(inten, .6f);

  inten_t const low = .25f, mid = .5f, high = .75f;
  if (inten < low) return qRgb(int(0xff * inten * 4), 0, 0);
  if (inten < mid) return qRgb(0xff, int(0xff * (inten - low) * 4), 0);
  if (inten < high)
