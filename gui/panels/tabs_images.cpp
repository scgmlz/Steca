...
void ImageWidget::setScale() {
  if (original_.isNull()) {
    ...
    scale_  = qMin(qreal(sz.width() - 2) / os.width(),
                  qreal(sz.height() - 2) / os.height());
  }
  ...
  if (hub_.actions.showOverlay->isChecked()) {
    ...
    QRect r   = rect.adjusted(-1, -1, 0, 0)
                  .adjusted(qRound(scale_ * cut.left), qRound(scale_ * cut.top),
                            -qRound(scale_ * cut.right),
                            -qRound(scale_ * cut.bottom));
    p.drawRect(r);
...
TabsImages::TabsImages(TheHub& hub) : super(hub) {
...

  {
    ...
    connect(numSlices_, slot(QSpinBox, valueChanged, int),
            [this]() { render(); });

    connect(numSlice_, slot(QSpinBox, valueChanged, int),
            [this]() { render(); });
...
void TabsImages::render() {
  {
    ...
    if (dataset_) {
      ...
      if (hub_.actions.showBins->isChecked()) {
        ...
        auto       curve =
            lens_->makeCurve(false);  // TODO factor out lens::binCount()
