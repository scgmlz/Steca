...
PanelGammaSlices::PanelGammaSlices(TheHub& hub) : super(hub, "Gamma slices") {
  ...
  connect(numSlices, slot(QSpinBox, valueChanged, int),
          [this]() { updateValues(); });
}
...
PanelInterpolation::PanelInterpolation(TheHub& hub)
    : super(hub, "Interpolation") {
  auto g = grid();
...
PanelDiagram::PanelDiagram(TheHub& hub) : super(hub, "Diagram") {
  ...
  for_i (data::Metadata::numAttributes(false) -
         data::Metadata::numAttributes(true))
...
Params::Params(TheHub& hub, ePanels panels)
    : RefHub(hub), panelReflection(nullptr), panelGammaSlices(nullptr),
      panelGammaRange(nullptr), panelPoints(nullptr),
  ...
  if (POINTS & panels) box_->addWidget((panelPoints = new PanelPoints(hub)));

  if (INTERPOLATION & panels)
    box_->addWidget((panelInterpolation = new PanelInterpolation(hub)));

  if (DIAGRAM & panels) box_->addWidget((panelDiagram = new PanelDiagram(hub)));
...
void Params::readSettings() {
  ...
  if (panelGammaRange)
    panelGammaRange->cbLimitGamma->setChecked(
        s.readBool(config_key::LIMIT_GAMMA, false));

  if (panelPoints)
    (s.readBool(config_key::INTERPOLATED, false) ? panelPoints->rbInterp
                                                 : panelPoints->rbCalc)
        ->setChecked(true);

  if (panelInterpolation) {
    panelInterpolation->stepAlpha->setValue(
        s.readReal(config_key::STEP_ALPHA, 5));
    panelInterpolation->stepBeta->setValue(
        s.readReal(config_key::STEP_BETA, 5));
    panelInterpolation->idwRadius->setValue(
        s.readReal(config_key::IDW_RADIUS, 10));

    panelInterpolation->avgAlphaMax->setValue(
        s.readReal(config_key::AVG_ALPHA_MAX, 15));
    panelInterpolation->avgRadius->setValue(
        s.readReal(config_key::AVG_RADIUS, 5));
    panelInterpolation->avgThreshold->setValue(
        s.readInt(config_key::AVG_THRESHOLD, 100));
  }

  saveDir = s.readStr(config_key::SAVE_DIR);
  saveFmt = s.readStr(config_key::SAVE_FMT);

  if (panelGammaSlices) panelGammaSlices->updateValues();

  if (panelGammaRange) panelGammaRange->updateValues();
}
...
void Params::saveSettings() const {
  ...
  if (panelGammaRange)
    s.saveBool(config_key::LIMIT_GAMMA,
               panelGammaRange->cbLimitGamma->isChecked());
  ...
  if (panelInterpolation) {
    ...
    s.saveReal(config_key::AVG_ALPHA_MAX,
               panelInterpolation->avgAlphaMax->value());
    s.saveReal(config_key::AVG_RADIUS, panelInterpolation->avgRadius->value());
    s.saveInt(config_key::AVG_THRESHOLD,
              panelInterpolation->avgThreshold->value());
  }
...
class TableModel : public models::TableModel {
  ...
  SUPER(models::TableModel)
      ...
      public : TableModel(TheHub&, uint numCols_);
...
QVariant TableModel::data(rcIndex index, int role) const {
  ...
  switch (role) {
  case Qt::DisplayRole:
    if (0 == indexCol) return rows_.at(to_u(indexRow)).n;
  ...
  case Qt::TextAlignmentRole:
    ...
    if (--indexCol < numCols && indexRow < numRows) {
      ...
      if (typ::isNumeric(var)) return Qt::AlignRight;
    }
...
QVariant TableModel::headerData(int section, Qt::Orientation, int role) const {
  if (section < 0 || headers_.count() < section) return QVariant();
...
void Table::setColumns(str_lst::rc headers, str_lst::rc outHeaders,
                       typ::cmp_vec::rc cmps) {
...
TabTable::TabTable(TheHub& hub, Params& params, str_lst::rc headers,
                   str_lst::rc outHeaders, typ::cmp_vec::rc cmps)
    : super(hub, params) {
  ...
  scrollArea->setWidget(
      (showColumnsWidget_ = new ShowColsWidget(*table, showCols_)));
...
TabTable::ShowColsWidget::ShowColsWidget(Table& table, showcol_vec& showCols)
  ...
  auto all = [this]() {
    for (auto& col : showCols_) col.cb->setChecked(true);
  };

  auto none = [this]() {
    for (auto& col : showCols_) col.cb->setChecked(false);
  };
...
TabSave::TabSave(TheHub& hub, Params& params, bool withTypes)
    : super(hub, params) {
  actBrowse = new TriggerAction("Browse...", this);
  actSave   = new TriggerAction("Save", this);

  str dir                      = params_.saveDir;
  if (!QDir(dir).exists()) dir = QDir::current().absolutePath();
  ...
  connect(actBrowse, &QAction::triggered, [this]() {
    ...
    if (!dir.isEmpty()) dir_->setText((params_.saveDir = dir));
  });
  ...
  connect(rbDat_, &QRadioButton::clicked,
          [this]() { params_.saveFmt = DAT_SFX; });

  connect(rbCsv_, &QRadioButton::clicked,
          [this]() { params_.saveFmt = CSV_SFX; });
...
str TabSave::filePath(bool withSuffix) {
  ...
  if (dir.isEmpty() || file.isEmpty()) return EMPTY_STR;

  str suffix;
  if (withSuffix) suffix = rbDat_->isChecked() ? DAT_SFX : CSV_SFX;
...
Frame::Frame(TheHub& hub, rcstr title, Params* params, QWidget* parent)
  ...
  if (params_->panelReflection) {
    connect(params_->panelReflection->cbRefl,
            slot(QComboBox, currentIndexChanged, int),
  ...
  auto tabTable = new TabTable(
      hub, *params_, calc::ReflectionInfo::dataTags(false),
      calc::ReflectionInfo::dataTags(true), calc::ReflectionInfo::dataCmps());
  ...
  if (pi) {
    ...
    for_i (calcPoints_.count())
      interpPoints_.append(
          calc::interpolate(calcPoints_.at(i), alphaStep, betaStep, idwRadius,
                            avgAlphaMax, avgRadius, avgTreshold, &progress));
  } else {
    for_i (calcPoints_.count())
...
void Frame::displayReflection(uint reflIndex, bool interpolated) {
  ...
  if (calcPoints_.count() <= reflIndex) return;
...
bool Frame::logCheckSuccess(rcstr path, bool res) const {
  if (!res) MessageLogger::popup(str("'%1' could not be saved.").arg(path));
  return res;
}
