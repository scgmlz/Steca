...
TabDiffractogramsSave::TabDiffractogramsSave(TheHub& hub, Params& params)
  ...
  g->addWidget((rbAllSequential_ = radioButton(
...
struct OutputData {
  ...
  OutputData(typ::Curve curve, data::Dataset dataset, gma_rge gmaStripe,
             uint picNum)
      : curve_(curve), dataset_(dataset), gmaStripe_(gmaStripe),
        picNum_(picNum) {}
  ...
  bool isValid() {
    return (!dataset_.metadata().isNull() || !curve_.isEmpty() ||
            gmaStripe_.isValid());
  }
};
...
DiffractogramsFrame::DiffractogramsFrame(TheHub& hub, rcstr title,
                                         QWidget* parent)
  ...
  connect(tabSave_->actSave, &QAction::triggered,
          [this]() { logSuccess(saveDiffractogramOutput()); });
}

OutputDataCollection
DiffractogramsFrame::collectCurves(gma_rge::rc rgeGma, uint gmaSlices,
...
OutputData DiffractogramsFrame::collectCurve(data::Dataset::rc dataset) {
  ...
  return OutputData(curve, dataset, lens->rgeGma(),
                    0);  // TODO current picture number
}
...
bool DiffractogramsFrame::writeCurrDiffractogramToFile(rcstr filePath,
                                                       rcstr separator) {
...
bool DiffractogramsFrame::writeAllDiffractogramsToFiles(rcstr filePath,
                                                        rcstr separator,
                                                        bool  oneFile) {
