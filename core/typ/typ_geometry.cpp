...
AngleMap::Key::Key(Geometry::rc geometry_, size2d::rc size_, ImageCut::rc cut_,
                   IJ::rc midPix_, tth_t midTth_)
...
void AngleMap::getGmaIndexes(gma_rge::rc rgeGma, uint_vec const*& indexes,
                             uint& minIndex, uint& maxIndex) const {
  indexes  = &gmaIndexes;
...
void AngleMap::calculate() {
  ...
  qreal d_midTth = midTth.toRad(), cos_midTth = cos(d_midTth),
        sin_midTth = sin(d_midTth);
  ...
  for (uint i = cut.left, iEnd = size.w - cut.right; i < iEnd; ++i) {
    for (uint j = cut.top, jEnd = size.h - cut.bottom; j < jEnd; ++j) {
      ...
      if (as.tth >= midTth) rgeGma_.extendBy(as.gma);  // gma range at mid tth
    }
  }
