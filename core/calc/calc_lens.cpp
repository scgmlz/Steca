...
LensBase::LensBase(core::Session::rc session, data::Datasets::rc datasets,
                   bool trans, bool cut, typ::ImageTransform::rc imageTransform,
                   typ::ImageCut::rc imageCut)
    : session_(session), datasets_(datasets), trans_(trans), cut_(cut),
...
size2d LensBase::transCutSize(size2d size) const {
  if (trans_ && imageTransform_.isTransposed()) size = size.transposed();

  if (cut_) size = size - imageCut_.marginSize();
...
ImageLens::ImageLens(core::Session::rc session, Image::rc image,
                     data::Datasets::rc datasets, bool trans, bool cut)
    : super(session, datasets, trans, cut, session.imageTransform(),
            session.imageCut()),
      image_(image) {}
...
inten_t ImageLens::imageInten(uint i, uint j) const {
  ...
  if (intensCorr_) inten *= intensCorr_->inten(i, j);

  return inten;
}

inten_rge::rc ImageLens::rgeInten(bool fixed) const {
  if (fixed) return datasets_.rgeFixedInten(session_, trans_, cut_);
...
DatasetLens::DatasetLens(core::Session::rc session, data::Dataset::rc dataset,
                         data::Datasets::rc datasets, eNorm norm, bool trans,
                         bool cut, typ::ImageTransform::rc imageTransform,
                         typ::ImageCut::rc imageCut)
...
void DatasetLens::setNorm(eNorm norm) {
  ...
  if (qIsNaN(normFactor_)) MessageLogger::warn("Bad normalisation value");
}
}
