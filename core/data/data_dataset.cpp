...
Metadata::Metadata()
    : date(), comment(), motorXT(0), motorYT(0), motorZT(0), motorOmg(0),
      motorTth(0), motorPhi(0), motorChi(0), motorPST(0), motorSST(0),
      motorOMGM(0), nmT(0), nmTeload(0), nmTepos(0), nmTeext(0), nmXe(0),
      nmYe(0), nmZe(0), monitorCount(0), deltaMonitorCount(0), time(0),
      deltaTime(0) {}

uint Metadata::numAttributes(bool onlyNum) {
  return uint(onlyNum ? eAttr::NUM_NUMERICAL_ATTRIBUTES
...
str_lst Metadata::attributeTags(bool out) {
  ...
  static str_lst const outTags = {
      "X",         "Y",     "Z",       "omega",  "mid2theta", "phi",
      "chi",       "PST",   "SST",     "OmegaM", "T",         "teload",
      "tepos",     "teext", "xe",      "ye",     "ze",        "mon",
      "delta_mon", "t",     "delta_t", "date",   "comment",
  };
...
typ::cmp_vec Metadata::attributeCmps() {
  ...
  static typ::cmp_vec const cmps = {
      cmp_real, cmp_real, cmp_real, cmp_real, cmp_real, cmp_real,
      cmp_real, cmp_real, cmp_real, cmp_real, cmp_real, cmp_real,
      cmp_real, cmp_real, cmp_real, cmp_real, cmp_real, cmp_real,
      cmp_real, cmp_real, cmp_real, cmp_date, cmp_str,
  };
...
typ::row_t Metadata::attributeNaNs() {
  ...
  if (row.isEmpty()) for_i (uint(eAttr::NUM_ALL_ATTRIBUTES))
      row.append(NAN);
  return row;
}
...
OneDataset::OneDataset(Metadata::rc md, size2d::rc size,
...
void OneDataset::collectIntens(core::Session::rc session,
                               typ::Image const* intensCorr, inten_vec& intens,
                               uint_vec& counts, gma_rge::rc rgeGma,
  ...
  for (uint i = gmaIndexMin; i < gmaIndexMax; ++i) {
    ...
    if (qIsNaN(inten)) continue;

    inten_t corr = intensCorr ? intensCorr->inten(ind) : 1;
    if (qIsNaN(corr)) continue;
    ...
    EXPECT(ti <= count)
    ti = qMin(ti,
              count - 1);  // it can overshoot due to floating point calculation
...
#define AVG_ONES(what)                        \
  EXPECT(!isEmpty())                          \
  qreal avg = 0;                              \
  for (auto& one : *this) avg += one->what(); \
  avg /= count();                             \
  return avg;
...
#define RGE_COMBINE(combineOp, what)                \
  EXPECT(!isEmpty())                                \
  Range rge;                                        \
  for (auto& one : *this) rge.combineOp(one->what); \
  return rge;
...
inten_vec Dataset::collectIntens(core::Session::rc session,
                                 typ::Image const* intensCorr,
                                 gma_rge::rc       rgeGma) const {
  ...
  for (auto& one : *this)
    one->collectIntens(session, intensCorr, intens, counts, rgeGma, minTth,
                       deltaTth);
...
inten_rge::rc Datasets::rgeGma(core::Session::rc session) const {
  if (!rgeGma_.isValid())
    for (auto& dataset : *this) rgeGma_.extendBy(dataset->rgeGma(session));

  return rgeGma_;
}

inten_rge::rc Datasets::rgeFixedInten(core::Session::rc session, bool trans,
                                      bool cut) const {
    ...
    for (auto& dataset : *this)
      for (auto& one : *dataset) {
        if (one->image()) {
          ...
          calc::shp_ImageLens imageLens =
              session.imageLens(image, *this, trans, cut);
...
Curve Datasets::avgCurve(core::Session::rc session) const {
  if (avgCurve_.isEmpty()) {
    ...
    avgCurve_ =
        session.datasetLens(*combineAll(), *this, session.norm(), true, true)
            ->makeCurve();
  }
...
shp_Dataset Datasets::combineAll() const {
  ...
  for (shp_Dataset const& dataset : *this)
    for (shp_OneDataset const& one : *dataset) d->append(one);
...
qreal Datasets::calcAvgMutable(qreal (Dataset::*avgMth)() const) const {
  ...
  if (!isEmpty()) {
    for (auto& dataset : *this) avg += ((*dataset).*avgMth)();
...
typ::shp_Image OneDatasets::foldedImage() const {
  ...
  for (auto& one : *this) image->addIntens(*one->image_);
