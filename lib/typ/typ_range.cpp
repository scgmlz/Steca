...
Range Range::intersect(Range::rc that) const {
  if (!isValid() || !that.isValid()) return Range();

  auto min_ = qMax(min, that.min), max_ = qMin(max, that.max);
  if (min_ <= max_) return Range(min_, max_);
...
TEST("Range::intersect", ({
       ...
       CHECK_EQ(Range::infinite(),
                Range::infinite().intersect(Range::infinite()));
...
qreal Range::bound(qreal value) const {
  if (isValid() && !qIsNaN(value)) return qBound(min, value, max);
  return NAN;
}
...
JsonArr Ranges::saveJson() const {
  ...
  for (auto& range : ranges_) arr.append(range.saveJson());
...
static bool RANGES_EQ(Ranges::rc rs, vec<min_max> mm) {
  if (rs.count() != mm.count()) return false;

  for_i (rs.count()) {
    auto r = rs.at(i);
    auto m = mm.at(i);
    if (r.min != m.min || r.max != m.max) return false;
  }
...
static bool RANGES_EQ(Ranges::rc rs1, Ranges::rc rs2) {
  if (rs1.count() != rs2.count()) return false;

  for_i (rs1.count()) {
    if (rs1.at(i) != rs2.at(i)) return false;
  }
