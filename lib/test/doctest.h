...
#define DOCTEST_STATIC_ASSERT(expression, message)                                                 \
    struct DOCTEST_CAT(__static_assertion_at_line_, __LINE__) {                                    \
        doctest::detail::static_assert_impl::StaticAssertion<static_cast<bool>((expression))>      \
            DOCTEST_CAT(                                                                           \
                DOCTEST_CAT(DOCTEST_CAT(STATIC_ASSERTION_FAILED_AT_LINE_, __LINE__), _), message); \
    };                                                                                             \
...
template <typename T> struct deferred_false { static const bool value = false; };
...
struct any_t {
    template <typename T> any_t(const DOCTEST_REF_WRAP(T));
};
...
template <> inline String toString<Approx>(const DOCTEST_REF_WRAP(Approx) value) {
...
struct TestData {
    ...
    TestData(const char* suite, const char* name, funcType f, const char* file, unsigned line)
        : m_suite(suite), m_name(name), m_f(f), m_file(file), m_line(line) {}
...
template <typename L> struct Expression_lhs {
    ...
    Expression_lhs(const Expression_lhs& other) : lhs(other.lhs) {}
...
void logAssert(
    bool passed, const char* decomposition, bool threw, const char* expr,
    assertType::Enum assert_type, const char* file, int line);

void logAssertThrows(
    bool threw, const char* expr, assertType::Enum assert_type, const char* file, int line);

void logAssertThrowsAs(
    bool threw, bool threw_as, const char* as, const char* expr, assertType::Enum assert_type,
...
// to fix gcc 4.7 "-Winline" warnings
    ...
    template <typename L> void unary_assert(const DOCTEST_REF_WRAP(L) val) {
...
template <int comparison, typename L, typename R>
int fast_binary_assert(
    assertType::Enum assert_type, const char* file, int line, const char* lhs_str,
    const char* rhs_str, const DOCTEST_REF_WRAP(L) lhs, const DOCTEST_REF_WRAP(R) rhs) {
...
template <typename L>
int fast_unary_assert(
    assertType::Enum assert_type, const char* file, int line, const char* val_str,
...
#define DOCTEST_TEST_CASE_FIXTURE(c, name)                                                         \
    DOCTEST_IMPLEMENT_FIXTURE(                                                                     \
        DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_), c, DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), name)
...
#define DOCTEST_SUBCASE(name)                                                                      \
    if (const doctest::detail::Subcase                                                             \
            & DOCTEST_ANONYMOUS(_DOCTEST_ANON_SUBCASE_)                                            \
                  __attribute__((unused)) = doctest::detail::Subcase(name, __FILE__, __LINE__))
#else // __GNUC__
#define DOCTEST_SUBCASE(name)                                                                      \
...
#define DOCTEST_TEST_SUITE(name)                                                                   \
    _Pragma("clang diagnostic push")                                                               \
        _Pragma("clang diagnostic ignored \"-Wglobal-constructors\"") static int                   \
            DOCTEST_ANONYMOUS(_DOCTEST_ANON_VAR_) = doctest::detail::setTestSuiteName(name);       \
    ...
    do {                                                                                           \
        _Pragma("clang diagnostic push")                                                           \
            _Pragma("clang diagnostic ignored \"-Woverloaded-shift-op-parentheses\"")              \
                DOCTEST_ASSERT_IMPLEMENT(expr, assert_type) _Pragma("clang diagnostic pop")        \
...
#define DOCTEST_ASSERT_THROWS_AS(expr, as, assert_type)                                            \
    do {                                                                                           \
        if (!DOCTEST_GCS().no_throw) {                                                             \
            doctest::detail::ResultBuilder _DOCTEST_RB(                                            \
                doctest::detail::assertType::assert_type, __FILE__, __LINE__, #expr, #as);         \
            try {                                                                                  \
                expr;                                                                              \
            } catch (as) {                                                                         \
...
#define DOCTEST_BINARY_ASSERT(assert_type, lhs, rhs, comp)                                         \
    do {                                                                                           \
        doctest::detail::ResultBuilder _DOCTEST_RB(                                                \
            doctest::detail::assertType::assert_type, __FILE__, __LINE__, #lhs ", " #rhs);         \
        DOCTEST_WRAP_IN_TRY(                                                                       \
...
#define DOCTEST_UNARY_ASSERT(assert_type, val)                                                     \
    do {                                                                                           \
        doctest::detail::ResultBuilder _DOCTEST_RB(                                                \
            doctest::detail::assertType::assert_type, __FILE__, __LINE__, #val);                   \
...
#define DOCTEST_FAST_BINARY_ASSERT(assert_type, lhs, rhs, comparison)                              \
    do {                                                                                           \
        int _DOCTEST_FAST_RES = doctest::detail::                                                  \
            fast_binary_assert<doctest::detail::binaryAssertComparison::comparison>(               \
...
#define DOCTEST_IMPLEMENT_FIXTURE(der, base, func, name)                                           \
    namespace {                                                                                    \
    template <typename T> struct der : base { void f(); };                                         \
    }                                                                                              \
    template <typename T> inline void der<T>::f()

#define DOCTEST_CREATE_AND_REGISTER_FUNCTION(f, name) template <typename T> static inline void f()
    ...
    do {                                                                                           \
        if (!DOCTEST_GCS().hasLoggedCurrentTestStart) {                                            \
            doctest::detail::logTestStart(                                                         \
                DOCTEST_GCS().currentTest->m_name, DOCTEST_GCS().currentTest->m_file,              \
...
template <typename T> T my_max(const T& lhs, const T& rhs) {
...
void freeStream(std::ostream* in) {
    delete in;
}
...
String::String(const char* in) : m_str(static_cast<char*>(malloc(detail::my_strlen(in) + 1))) {
    if (in)
        strcpy(m_str, in);
    else
        m_str[0] = '\0';
...
unsigned String::size() const {
    return m_str ? detail::my_strlen(m_str) : 0;
}
...
String toString(float in) {
    return detail::fpToString(in, 5) + "f";
}
String toString(double in) {
    return detail::fpToString(in, 10);
}
String toString(double long in) {
    return detail::fpToString(in, 15);
}
...
int matchesAny(const char* name, std::vector<String> filters, int matchEmpty, bool caseSensitive) {
...
Subcase::Subcase(const Subcase& other)
    : m_signature(other.m_signature.m_name, other.m_signature.m_file, other.m_signature.m_line)
...
bool ResultBuilder::log() {
    ...
    if (m_failed || DOCTEST_GCS().success) {
        ...
        if (m_assert_type & assertType::is_throws) {
            ...
            logAssertThrowsAs(
                m_threw, m_threw_as, m_exception_type, m_expr, m_assert_type, m_file, m_line);
        } else if (m_assert_type & assertType::is_nothrow) {
            logAssertNothrow(m_threw, m_expr, m_assert_type, m_file, m_line);
        } else {
            logAssert(
                m_result.m_passed, m_result.m_decomposition.c_str(), m_threw, m_expr, m_assert_type,
                m_file, m_line);
        }
    }
...
bool parseCommaSepArgs(
    int argc, const char* const* argv, const char* pattern, std::vector<String>& res) {
...
//}
    ...
    if (p->count || p->list_test_cases || p->list_test_suites) {
        ...
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "%4d passed", numTestsPassingFilters - numFailed);
        ...
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "%4d skipped\n",
        ...
        DOCTEST_PRINTF_COLORED(buff, Color::None);
        DOCTEST_SNPRINTF(
            buff, DOCTEST_COUNTOF(buff), "%4d passed", p->numAssertions - p->numFailedAssertions);
        DOCTEST_PRINTF_COLORED(buff, numFailed > 0 ? Color::None : Color::Green);
        DOCTEST_SNPRINTF(buff, DOCTEST_COUNTOF(buff), " | ");
        DOCTEST_PRINTF_COLORED(buff, Color::None);
