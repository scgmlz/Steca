...
TEST("Exception()", CHECK_THROWS_AS(THROW("Hello"), Exception);
     CHECK_THROWS_AS(THROW_SILENT(), Exception);

     CHECK_NOTHROW(Exception()); CHECK_THROWS_AS(Exception().raise(), Exception);

     scoped<Exception*> p(Exception().clone()); CHECK_THROWS_AS(p->raise(), Exception);)

Exception::Exception(rcstr msg) noexcept : Cls(msg, false) {}

TEST("Exception(\"Hi\")", try { THROW("Hi"); } catch (Exception& e) {
    CHECK_EQ("Hi", e.msg());
    return;
}
