...
#define TEST_LENS \
 Session session; \
  ...
  \
  Lens lensNoTrans(session,*(dataset.data()),&corr, Range(),true,true,core::eNorm::NONE,\
...
void TestCoreLens::testNormLens() {
  ...
  {
    {
      Lens lensNorm(session,*(dataset.data()),&corr,Range(),true,true,
    ...
    {
      Lens lensNorm(session,*(dataset.data()),&corr,Range(),true,true,core::eNorm::DELTA_TIME,
      angleMap,ImageCut(),ImageTransform::ROTATE_0);
...
void TestCoreLens::testTransformationLens() {
  ...
  TEST_LENS
  { 
    Lens lensTrans(session,*(dataset.data()),&corr,Range(),true,true,core::eNorm::NONE,
...
void TestCoreLens::testCutLens() {
  ...
  TEST_LENS
  {
    Lens lensCut(session,*(dataset.data()),&corr,Range(),true,true,core::eNorm::NONE,
...
void TestCoreLens::testSensitivityCorrectionLens() {
  ...
  TEST_LENS
  {
    Lens lensCorr(session,*(dataset.data()),&corr,Range(),true,true,core::eNorm::NONE,
    angleMap,ImageCut(),ImageTransform::ROTATE_0);
...
void TestCoreLens::testGlobalIntensityRangeLens() {
  ...
  auto range = session.collectedDatasets().rgeFixedInten(session,true,true);

  Lens lensGlobRange(session,*(dataset.data()),&corr,range,true,true,core::eNorm::NONE,
