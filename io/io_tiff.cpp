...
#define IS_NUMBER                                                    \
  RUNTIME_CHECK((1 == dataType || 3 == dataType || 4 == dataType) && \
                    1 == dataCount,                                  \
                BAD_FORMAT)
...
static void loadTiff(data::shp_File& file, rcstr filePath, typ::deg phi,
                     qreal monitor, qreal expTime) THROWS {
  ...
  quint16 version;
  is >> version;
  if (42 != version) THROW("bad version");
  ...
  RUNTIME_CHECK(imageWidth > 0 && imageHeight > 0 && stripOffsets > 0 &&
                    stripByteCounts > 0 && imageHeight <= rowsPerStrip,
                "bad format");

  RUNTIME_CHECK((1 == sampleFormat || 2 == sampleFormat || 3 == sampleFormat) &&
                    32 == bitsPerSample,
                "unhandled format");
